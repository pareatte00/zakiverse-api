...
func (r *Repository) FindDueRotations(ctx context.Context, now time.Time) ([]model.PackPool, error) {
	...
	stmt := postgres.SELECT(PackPool.AllColumns).
		FROM(PackPool).
		WHERE(
			PackPool.RotationType.NOT_EQ(postgres.NewEnumValue("none")).
				AND(PackPool.NextRotationAt.IS_NOT_NULL()).
				AND(PackPool.NextRotationAt.LT_EQ(postgres.TimestampzT(now))),
		)
